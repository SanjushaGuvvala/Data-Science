...
for (i in c("BTC", "ETH", "XRP", "BCH", "LTC")){
  data <- subset(CryptoCurrencies, symbol == i & date >= "2017-04-01" & date <= "2018-03-25",frequency=12)
...
# Check the cycles in time series
class(TSData)
frequency(TSData)         # frequency of seasons
cycle(TSData)             # cycles for seasons
time(TSData)              # time intervals


bitcoin <- subset(data, symbol =="BTC")
ethereum <- subset(data, symbol =="ETH")
xrp <- subset(data, symbol =="XRP")
bitcoin_cash <- subset(data, symbol =="BCH")
litecoin <- subset(data, symbol =="LTC")

for (i in c("BTC", "ETH", "XRP", "BCH", "LTC")){
  subset_data <- subset(cryptoData1, symbol ==i)
  crypto_ts <- ts(subset_data,start=c(2016,11), end=c(2018,11), frequency=365)  
  autoplot(crypto_ts[,3], main = "Open Price Distribution of Bitcoin", 
           xlab = "Date", ylab = "Open Prices in USD")
}
